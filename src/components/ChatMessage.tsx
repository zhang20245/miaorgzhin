...
// Helper types for other markdown components
...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  const markdownComponents: Components = {
    ...
    p: ({ children }: ChildrenProps) => (
      <p className="my-2 animate-fade-in">{children}</p>
    ),
    h1: ({ children }: ChildrenProps) => (
      <h1 className="text-2xl font-bold my-4 animate-slide-in">{children}</h1>
    ),
    h2: ({ children }: ChildrenProps) => (
      <h2 className="text-xl font-bold my-3 animate-slide-in">{children}</h2>
    ),
    h3: ({ children }: ChildrenProps) => (
      <h3 className="text-lg font-bold my-2 animate-slide-in">{children}</h3>
    ),
    ul: ({ children }: ChildrenProps) => (
      <ul className="list-disc ml-4 my-2 animate-fade-in">{children}</ul>
    ),
    ol: ({ children }: ChildrenProps) => (
      <ol className="list-decimal ml-4 my-2 animate-fade-in">{children}</ol>
    ),
    li: ({ children }: ChildrenProps) => (
      <li className="my-1 animate-slide-in">{children}</li>
    ),
